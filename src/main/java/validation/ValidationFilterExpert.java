...
public class ValidationFilterExpert {
    public static String isValidInfo(String input) {
        String check, checkNull, name = null, family = null, email = null, duty = null;
        ...
        if (isValidLength(split.length)) {
            ...
            checkNull = isValidCharacter(split[3]);
            if (checkNull.equals("true")) {
                duty = split[1];
            }
            check = name + "," + family + "," + email + "," + duty;
            if (check.length() > 1)
                return check;
